'use strict';
const VERSION='mobile-builder-v20-direct-page';
...
async function networkFirst(request){try{const res=await fetch(request,{cache:'no-store'});if(!res.ok)return res;const c=await caches.open(VERSION);c.put(request,res.clone()).catch(()=>{});return res}catch{const hit=await caches.match(request);return hit||new Response('',{status:504})}}
