'use strict';
const VERSION='mobile-builder-v16-background-clouds';
...
async function patchHTML(response){
  ...
  html=html.replace('scene.background=new THREE.Color(0xb9c1c6);scene.fog=new THREE.Fog(0xb9c1c6,70,180);','scene.background=new THREE.Color(0x87ceeb);scene.fog=new THREE.Fog(0x87ceeb,180,420);');
  const sky=`
  const skySun=new THREE.Mesh(new THREE.SphereGeometry(8,24,24),new THREE.MeshBasicMaterial({color:0xfff4b0}));
  skySun.position.set(-55,70,-260);scene.add(skySun);
  const cloudMat=new THREE.MeshLambertMaterial({color:0xffffff,transparent:true,opacity:.88});
  const cloudData=[[-145,72,-300,1.0],[-75,58,-235,.72],[15,76,-340,.9],[105,62,-275,.78],[175,82,-390,1.05],[-185,48,-420,.68],[-20,52,-455,.72],[75,92,-430,.82],[210,58,-470,.65],[-250,88,-360,.8]];
