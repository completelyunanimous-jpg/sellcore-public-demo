'use strict';
const VERSION='3dmobile-v1';
const CACHE_PREFIX='3dmobile-';
...
self.addEventListener('fetch',event=>{if(event.request.method!=='GET'||new URL(event.request.url).origin!==self.location.origin)return;if(event.request.mode==='navigate'){event.respondWith(networkFirst(event.request));return}if(event.request.url.includes('/3d-mobile/'))event.respondWith(networkFirst(event.request))});
