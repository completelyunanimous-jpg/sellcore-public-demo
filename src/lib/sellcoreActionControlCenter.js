...
export function resolveAutomationSignal(type) {
  const signals = {
    corefix_bridge_started: "corefix_bridge_ready",
    corefix_bridge_ping: "corefix_bridge_alive",
    corefix_card_created: "corefix_card_logged",
...
export function recordSellCoreAction(type, payload = {}) {
  ...
  safeWrite(ACTION_KEY, [action].concat(actions).slice(0, MAX_ACTIONS));
  return action;
}
